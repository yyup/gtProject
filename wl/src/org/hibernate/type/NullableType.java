...
public abstract class NullableType extends AbstractType {
	 ...
	 * <p/>
	 * Originally was cached such because of performance of looking up the logger each time in order to check the trace-enablement. Driving this via a central Log-specific class would alleviate that performance hit, and yet still allow more "normal"
	...
	public final void nullSafeSet(PreparedStatement st, Object value, int index) throws HibernateException, SQLException {
		try {
			if (value == null) {// 原为"value==null|| rs.wasNull()"为了解决数值型为空，查询出问题修改
