 ...
 * 
 */
@Repository("wl.cm.WlCmSellDao")
public class WlCmSellDao extends BaseDao {

	public WlCmSellDao() {
		modelClass = WlCmSell.class;
		defaultOrder = "";
	}
	...
	public Page findSellsPages(int pageSize, int currPage, String sellStateEk, String typeId, String productName, String isThumFlag, String sellIds, String filterSellIds, String auditNodeEk, String auditState) {
		try {
			...
			if (!ValidateUtil.isEmpty(isThumFlag)) {
				...
				hql += "from WlCmSell as s,WlCmProductType as p,WlCmSellPic as w where 1=1 and s.typeId=p.typeId  ";
				...
				hql += "from WlCmSell as s,WlCmProductType as p where 1=1 and s.typeId=p.typeId  ";
			}
	...
	public List findSellListByIds(String sellIds){
		try {
			...
			hql += " from WlCmSell as s,WlCmProductType as p where s.typeId=p.typeId ";
			if (!ValidateUtil.isEmpty(sellIds)) {
				hql += "and s.sellId in" + sellIds;
			}
			return this.find(hql);
	...
	public void removeData(String strId) {
		try {
			...
			String hql = "delete from WlCmSell  a where " + hqlWhere;
			this.executeHql(hql);
		}
		catch (BaseException e) {
			throw new BaseException(e, log);
	...
	public int getSellCountBySellIdsAndState(String sellIds, String sellState) {
		try {
			...
			String hql = "from WlCmSell where sellStateEk='" + sellState + "' and " + hqlWhere;
	...
	public List findTpEsSellBySellIdsAndState(String sellIds, String sellState) {
		try {
			...
			return this.findByFreeSQL(sql, WlCmSell.class);
		}
		catch (Exception e) {
			throw new BaseException(e, log);
		}
	...
	public List findEsSellByState(String sellState) {
		try {
			...
			String sql = "SELECT m.* from WL_CM_SELL m where 1=1 ";
	...
	public void updateState(String strHql, String ek, String sellTime) {
		try {
			String hql = " update WlCmSell set sellStateEk =" + FormatUtil.formatStrForDB(ek);
	...
	public WlCmSell getWlCmSellByItemCd(String itemCd) {
		try {
			String hql = "from WlCmSell where 1=1 ";
