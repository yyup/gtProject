 ...
 *
 */
@Repository("wl.cm.WlCmSellAllocationEnDao")
public class WlCmSellAllocationEnDao extends BaseDao {

	public WlCmSellAllocationEnDao() {
		modelClass = WlCmSellAllocationEn.class;
		defaultOrder = "";
	}
	/**
	 * 查询配置列表
	...
	public List findAllocationList(String parentSellId){
		try {
			String hql="from WlCmSellAllocationEn where 1=1 ";
	...
	public List findSellAllocationList(String sellId) {
		try {
			...
			hql += " from WlCmSellAllocationEn sl ,WlCmSellPicEn sp ,WlCmSellEn s";
	...
	public void removeData(String sellId,String parentSellId) {
		try {
			if (!ValidateUtil.isEmpty(sellId)||!ValidateUtil.isEmpty(parentSellId)) {
				...
				String hql = "delete from WlCmSellAllocationEn  a where " + hqlWhere;
				this.executeHql(hql);
			}
		}
		catch (BaseException e) {
