...
@Repository("wl.cm.WlCmProductTypeDao")
public class WlCmProductTypeDao extends BaseDao {

	public WlCmProductTypeDao() {
		modelClass = WlCmProductType.class;
		defaultOrder = "";
	}
	...
	public List<WlCmProductType> findAllTypeList() {
		try {
			String hql = "from WlCmProductType  where dataState ='1' ";
			hql += "order by sequ";
	...
	public List<WlCmProductType> findChildTypeList(String parentTypeId) {
		try {
			String hql = "from WlCmProductType  where dataState ='1' and parentTypeId =? ";
	...
	public List<WlCmProductType> findAllLastType(String dataState) {
		...
		try {
			String hql = " from WlCmProductType m where m.typeId not in (select s.parentTypeId from WlCmProductType s)";
	...
	public void deleteProductType(String[] ids) {
		String hql = "update WlCmProductType set dataState ='0' where typeId in " + ConvertUtil.toDbString(ids);
		try {
			this.executeHql(hql);
		}
		catch (Exception e) {
