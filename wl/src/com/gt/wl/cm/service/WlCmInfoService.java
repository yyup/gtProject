...
import com.gt.wl.cm.dao.WlCmInfoDao;
 ...
 * 
 */
@Service("wl.cm.WlCmInfoService")
public class WlCmInfoService extends BaseService {
	private WlCmInfoDao wlCmInfoDao = (WlCmInfoDao) Sc.getBean("wl.cm.WlCmInfoDao");
	...
	private static Map<String, WlCmColumn> columnMap = new LinkedHashMap<String, WlCmColumn>();

	public WlCmInfoService() {
		baseDao = wlCmInfoDao;
	}
	...
	public void saveColumnToMap(String smc) {
		try {
			List<WlCmColumn> list = wlCmColumnService.findAllColumnList("1", "","");
			...
			if ("1".equals(smc)) {// 初审
				// 查找末级节点
				List<WlCmColumn> lastList = wlCmColumnService.findAllColumnList("1", "1","");
				for (WlCmColumn lastWlCmColumn : lastList) {
					long firstAuditNum = wlCmInfoDao.getInfoCount(lastWlCmColumn.getColumnId(), "1", "0");
					if (firstAuditNum != 0) {
						updateColumnMapFirstAuditNum(lastWlCmColumn.getColumnId(), firstAuditNum);
					}
				}
			}
				...
				List<WlCmColumn> lastList = wlCmColumnService.findAllColumnList("1", "1","");
				for (WlCmColumn lastWlCmColumn : lastList) {
					long finalAuditNum = wlCmInfoDao.getInfoCount(lastWlCmColumn.getColumnId(), "2", "0");
					if (finalAuditNum != 0) {
						updateColumnMapFinalAuditNum(lastWlCmColumn.getColumnId(), finalAuditNum);
					}
				}
			}
	...
	public void updateColumnMapFirstAuditNum(String columnId, long childFirstAuditNum) {
		// 查找初审未审核数量
		try {
			WlCmColumn wlCmColumn = columnMap.get(columnId);
	...
	public List findcolumnMapToList(String smc) {
		try {
			List<WlCmColumn> list = new ArrayList();
			for (WlCmColumn wlCmColumn : columnMap.values()) {
				if ("1".equals(smc)) {
		...
		try {
			if ("2".equals(issueStateEk)) {// 要发布
				...
				if (!ValidateUtil.isEmpty(list)) {// 勾选的数据中含有已发布的，不能发布
					throw new BaseException(Lang.getString("wl.cm.WlCmInfoService.notAllIssue"));
				}
				else {
					list = wlCmInfoDao.findInfoList(ConvertUtil.toDbString(infoIds),"", "", "2", "3","","");
					if (infoIds.length != list.size()) {// 勾选的数据不是全部都终审通过的，不能发布
						throw new BaseException(Lang.getString("wl.cm.WlCmInfoService.notFinalPassIssue"));
					}
				}
			...
			for (String infoId : infoIds) {
				WlCmInfo wlCmInfo = (WlCmInfo) this.getObject(infoId);
	 ...
	 * @param wlCmInfo 信息对象
	 */
	public void saveData(WlCmInfo wlCmInfo, User user) {
		try {
	...
	public void updateFirstAudit(String[] infoIds, String auditRemark, String auditNodeEk, String auditState, User user) {
		...
		if (!ValidateUtil.isEmpty(infoIds)) {
			...
			if (list.size() != infoIds.length) {// 勾选的数据中并非都是初审未审核
				throw new BaseException(Lang.getString("wl.cm.WlCmInfoService.noAllFirstAudit"));
			}
	...
	public void updateFinalAudit(String[] infoIds, String auditRemark, String auditNodeEk, String auditState, String columnType, User user) {
		...
		if (!ValidateUtil.isEmpty(infoIds)) {
			...
			if (list.size() != infoIds.length) {// 勾选的数据中并非都是终审未审核
				throw new BaseException(Lang.getString("wl.cm.WlCmInfoService.noAllFinalAudit"));
			}
			String auditOperateEk = "";
