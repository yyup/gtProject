...
@Controller
@RequestMapping({"/wl/cm/wlCmProductTypeAction.do","/wl/cm/wlCmProductTypeAction.dox"})
public class WlCmProductTypeAction extends BaseAction {
	...
	private final static String MODULE_NAME = Lang.getString("wl.cm.WlCmProductTypeAction.MODULE_NAME");
	private final static String MODULE_CODE = "0301007";// 模块代码
	...
	@ResponseBody
	public String saveData(@ModelAttribute("saveModel") WlCmProductType wlCmProductType, User user) {
	...
	public String deleteData(String ids[], User user) {
		...
		try {
			for (int i = 0; i < ids.length; i++) {
				// 获取类型的名称
				WlCmProductType type = (WlCmProductType) wlCmProductTypeService.getObject(ids[i]);
	...
	public String getData(String typeId) {
		try {
			// 根据ID得到商品类型对象
			WlCmProductType wlCmProductType = (WlCmProductType) wlCmProductTypeService.getObject(typeId);
			// 获取类型ID对应的父类型名称
			WlCmProductType parentType = (WlCmProductType) wlCmProductTypeService.getObject(wlCmProductType.getParentTypeId());
