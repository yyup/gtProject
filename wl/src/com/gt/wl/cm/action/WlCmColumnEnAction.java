 ...
 */
@Controller
@RequestMapping({ "/wl/cm/wlCmColumnEnAction.do", "/wl/cm/wlCmColumnEnAction.dox" })
public class WlCmColumnEnAction extends BaseAction {
	private final static String SYSTEM_NAME = Lang.getString("wl.cm.SYSTEM_NAME");
	private final static String MODULE_NAME = Lang.getString("wl.cm.WlCmColumnEnAction.MODULE_NAME");
	...
	private final static String MODULE_CODE = "0307001";// 模块代码
	...
	public String getCenterServiceContent(User user, String isEnableFlag, String isLastFlag, String rootId) {
		try {
			...
			List<WlCmColumnEn> childList;//获取下级列表
			if(!ValidateUtil.isEmpty(list)){//部位空获取第一个
				WlCmColumnEn parentWlCmColumnEn = (WlCmColumnEn) list.get(0);
				childList = wlCmColumnService.findAllColumnList(isEnableFlag, isLastFlag,parentWlCmColumnEn.getColumnId());
				map.put("titleList", childList);
				for (WlCmColumnEn child : childList) {
					List<WlCmInfoEn> infoList = wlCmInfoService.findInfoList("",child.getColumnId(),"2","","","1","");
					child.setChildColumnList(infoList);
					for (WlCmInfoEn wlCmInfo : infoList) {
	...
	@ResponseBody
	public String saveData(WlCmColumnEn wlCmColumn, User user) {
	...
	public String getData(String columnId) {
		try {
			// 根据ID得到栏目类型对象
			WlCmColumnEn wlCmColumn = (WlCmColumnEn) wlCmColumnService.getObject(columnId);
			// 获取类型ID对应的父类型名称
			WlCmColumnEn parentColumn = (WlCmColumnEn) wlCmColumnService.getObject(wlCmColumn.getParentColumnId());
	...
	public String getChildData(String columnId) {
		try {
			...
			List<WlCmColumnEn> childTypeList = wlCmColumnService.findChildColumnList(columnIds, "1", "1");
			for (WlCmColumnEn wlCmColumn : childTypeList) {//栏目下面的信息
				List<WlCmInfoEn> infoList = wlCmInfoService.findInfoList("",wlCmColumn.getColumnId(),"2","","","1","");
	...
	public String deleteData(String ids[], User user) {
		...
		try {
			for (int i = 0; i < ids.length; i++) {
				// 获取类型的名称
				WlCmColumnEn wlCmColumn = (WlCmColumnEn) wlCmColumnService.getObject(ids[i]);
	...
	@ResponseBody
	public String updateWlCmColumnEnState(String[] ids, String isEnableFlag, User user) {
		try {
