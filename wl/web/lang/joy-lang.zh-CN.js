(function(window, undefined) {
	...
	joy.pluginLang = {
		...
		textbox:{
			...
			telRule:/^(((\d+\-?\d*)+\,?)*|())$/,
			mobileTel:"手机号码格式有误",
			mobileTelRule:/^((0|86|17951)?(13[0-9]|15[012356789]|17[0-9]|18[0-9]|14[57])[0-9]{8}|())$/,
			idCard:"身份证号码格式有误",
			idCardRule:/^((QT-\S+)|(\d{15})|((\d{17})(X|x|\d{1}))|())$/, 
			entCode:"法人代码格式有误，如：435834567(9位)",
			entCodeRule:/^([a-zA-Z0-9]{9}|())$/,
			numCode:"编号格式有误，请输入-字母或数字",
			numCodeRule:/^[0-9a-zA-Z\-]*$/,
		...
		uploadEn:{
            ...
            "headerStatusUploaded": "uploaded",
            "maxSize":"the size of the selected file exceeds the limit, and the size must not exceed：",
            "allowType":"upload file format is not up to specification, support format is：",
            "maxNameSize":"the name of the file shall not exceed the length of the file："
		},
		common:{
			ajaxError:"服务请求出错"
		},
		...
		imagebrower:{
				messages:{
	                ...
	                emptyFolder: "空文件夹",
	                deleteFile: '确定删除 "{0}"?',
	                invalidFileType: "文件 \"{0}\" 格式不符。文件类型应为 {1}。",
	                overwriteFile: "文件名 \"{0}\" 在文件夹中已经存在. 确定覆盖它?",
	                dropFilesHere: "拖动文件到这里上传",
	                search: "搜索"
                }
		}
